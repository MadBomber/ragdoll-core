...
class CreateActiveStorageTables < ActiveRecord::Migration[7.0]
  def change
    ...
    create_table :active_storage_blobs, comment: 'File metadata and storage backend information for uploaded documents' do |t|
      ...
      t.string :key, null: false, comment: 'Unique storage identifier preventing enumeration, format: [random]/[random]'
      
      # Original filename as provided during upload
      # Preserved for display purposes and content type detection
      t.string :filename, null: false, comment: 'Original filename from upload, used for display and content type detection'
      
      # MIME content type of the uploaded file
      # Used for content validation and processing strategy selection
      t.string :content_type, comment: 'MIME content type for validation and processing strategy (application/pdf, text/plain, etc.)'
      ...
      t.text :metadata, comment: 'JSON metadata including document properties, processing info, and custom attributes'
      
      # Service name indicating which storage backend is used
      # Examples: 'local', 's3', 'gcs', 'azure' - determined by Rails configuration
      t.string :service_name, null: false, comment: 'Storage backend identifier (local, s3, gcs, azure) from Rails configuration'
      
      # File size in bytes - used for validation, storage management, and display
      t.bigint :byte_size, null: false, comment: 'File size in bytes for validation, storage management, and UI display'
    ...
    create_table :active_storage_attachments, comment: 'Polymorphic join table connecting models to blob files, Documents to their attached files' do |t|
    ...
    create_table :active_storage_variant_records, comment: 'Image variant metadata for thumbnails and resized versions, optional for document previews' do |t|
      ...
      t.string :variation_digest, null: false, comment: 'Serialized transformation key for thumbnail/preview generation'
    ...
    add_index :active_storage_blobs, [:key], unique: true, comment: 'Unique index on storage key for fast blob lookup' unless index_exists?(:active_storage_blobs, [:key], unique: true)
    
    # ActiveStorage attachment indexes
    add_index :active_storage_attachments, [ :record_type, :record_id, :name, :blob_id ], 
              name: "index_active_storage_attachments_uniqueness", unique: true,
              comment: 'Composite unique index ensuring one attachment per model+name combination' unless index_exists?(:active_storage_attachments, [ :record_type, :record_id, :name, :blob_id ], name: "index_active_storage_attachments_uniqueness")
              
    add_index :active_storage_attachments, [:blob_id], 
              comment: 'Index for finding all attachments using a specific blob (reverse lookup)' unless index_exists?(:active_storage_attachments, [:blob_id])
              
    add_index :active_storage_attachments, [:record_type, :record_id, :name], 
              name: "index_active_storage_attachments_on_record",
              comment: 'Optimized index for Ragdoll document file attachment lookups' unless index_exists?(:active_storage_attachments, [:record_type, :record_id, :name], name: "index_active_storage_attachments_on_record")
    ...
    add_index :active_storage_variant_records, [ :blob_id, :variation_digest ], 
              ...
              comment: 'Composite unique index ensuring one variant per blob+transformation combination' unless index_exists?(:active_storage_variant_records, [ :blob_id, :variation_digest ], name: "index_active_storage_variant_records_uniqueness")
    ...
    when 'mysql2'
      ...
      add_index :active_storage_blobs, :filename, type: :fulltext, name: 'index_active_storage_blobs_on_filename_fulltext'
    end
  end
  ...
  def down
    ...
    remove_index :active_storage_attachments, name: "index_active_storage_attachments_uniqueness" if index_exists?(:active_storage_attachments, [ :record_type, :record_id, :name, :blob_id ], name: "index_active_storage_attachments_uniqueness")
    remove_index :active_storage_attachments, [:blob_id] if index_exists?(:active_storage_attachments, [:blob_id])
    remove_index :active_storage_attachments, name: "index_active_storage_attachments_on_record" if index_exists?(:active_storage_attachments, [:record_type, :record_id, :name], name: "index_active_storage_attachments_on_record")
    remove_index :active_storage_variant_records, name: "index_active_storage_variant_records_uniqueness" if index_exists?(:active_storage_variant_records, [ :blob_id, :variation_digest ], name: "index_active_storage_variant_records_uniqueness")
    
    # Clean up database-specific optimizations
    case connection.adapter_name.downcase
    when 'postgresql'
      execute "DROP INDEX IF EXISTS index_active_storage_blobs_on_metadata;" rescue nil
    when 'mysql2'
      remove_index :active_storage_blobs, name: 'index_active_storage_blobs_on_filename_fulltext' if index_exists?(:active_storage_blobs, :filename, name: 'index_active_storage_blobs_on_filename_fulltext')
    end
