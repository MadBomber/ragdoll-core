...
class CreateRagdollEmbeddings < ActiveRecord::Migration[7.0]
  def change
    create_table :ragdoll_embeddings,
                 comment: 'Vector embeddings storage for semantic similarity search with usage tracking and model metadata' do |t|
      ...
      t.text :content, null: false,
                       comment: 'Original text content that was embedded, typically a document chunk'
      ...
      t.json :embedding_vector, null: false,
                                comment: 'Vector embedding as JSON array, cross-database compatible numerical representation'
      ...
      t.string :model_name, null: false,
                            comment: 'Embedding model identifier (e.g. text-embedding-3-small), critical for query compatibility'
      ...
      t.integer :dimensions, null: false,
                             comment: 'Vector dimensionality (e.g. 1536, 3072), must match model specifications'
      ...
      t.integer :start_position, default: 0,
                                 comment: 'Starting character position within source document for chunking and highlighting'
      ...
      t.integer :end_position, default: 0,
                               comment: 'Ending character position within source document, defines exact text span'
      ...
      t.integer :usage_count, default: 0,
                              comment: 'Number of times used in similarity searches, for caching optimization'
      ...
      t.datetime :last_used_at,
                 comment: 'Timestamp of most recent usage, for recency-based ranking and cache management'
      ...
      t.float :similarity_score,
              comment: 'Optional similarity or confidence score for embedding quality tracking'
      ...
      t.json :metadata, default: {},
                        comment: 'Flexible JSON storage for embedding metadata, processing parameters, custom attributes'
    ...
    unless index_exists?(
      :ragdoll_embeddings, :document_id
    )
      add_index :ragdoll_embeddings, :document_id,
                comment: 'Index for finding embeddings by document'
    end
    unless index_exists?(
      :ragdoll_embeddings, :model_name
    )
      add_index :ragdoll_embeddings, :model_name,
                comment: 'Index for filtering by embedding model'
    end
    unless index_exists?(
      :ragdoll_embeddings, %i[model_name usage_count]
    )
      add_index :ragdoll_embeddings, %i[model_name usage_count],
                comment: 'Composite index for model-specific usage ranking'
    end
    unless index_exists?(
      :ragdoll_embeddings, %i[model_name dimensions]
    )
      add_index :ragdoll_embeddings, %i[model_name dimensions],
                comment: 'Composite index for model and dimension validation'
    end
    unless index_exists?(
      :ragdoll_embeddings, :last_used_at
    )
      add_index :ragdoll_embeddings, :last_used_at,
                comment: 'Index for recency-based queries and cache management'
    end
    unless index_exists?(
      :ragdoll_embeddings, :usage_count
    )
      add_index :ragdoll_embeddings, :usage_count,
                comment: 'Index for popularity-based ranking and optimization'
    end
    unless index_exists?(
      :ragdoll_embeddings, %i[document_id start_position]
    )
      add_index :ragdoll_embeddings, %i[document_id start_position],
                comment: 'Composite index for ordered content chunks within documents'
    end
  ...
  def down
    ...
    remove_index :ragdoll_embeddings, %i[model_name usage_count] if index_exists?(:ragdoll_embeddings,
                                                                                  %i[model_name usage_count])
    remove_index :ragdoll_embeddings, %i[model_name dimensions] if index_exists?(:ragdoll_embeddings,
                                                                                 %i[model_name dimensions])
    ...
    remove_index :ragdoll_embeddings, %i[document_id start_position] if index_exists?(:ragdoll_embeddings,
                                                                                      %i[document_id
                                                                                         start_position])
    ...
    when 'postgresql'
      if column_exists?(:ragdoll_embeddings, :embedding_vector_native)
        remove_index :ragdoll_embeddings, name: 'index_ragdoll_embeddings_on_vector_cosine' if index_exists?(
          :ragdoll_embeddings, :embedding_vector_native, name: 'index_ragdoll_embeddings_on_vector_cosine'
        )
    ...
    when 'sqlite'
      ...
      begin
        execute 'DROP TRIGGER IF EXISTS ragdoll_embeddings_vec_insert;'
      rescue StandardError
        nil
      end
      begin
        execute 'DROP TRIGGER IF EXISTS ragdoll_embeddings_vec_update;'
      rescue StandardError
        nil
      end
      begin
        execute 'DROP TRIGGER IF EXISTS ragdoll_embeddings_vec_delete;'
      rescue StandardError
        nil
      end
      begin
        execute 'DROP TABLE IF EXISTS ragdoll_embeddings_vec;'
      rescue StandardError
        nil
      end
    when 'mysql2'
      remove_column :ragdoll_embeddings, :embedding_vector_binary if column_exists?(:ragdoll_embeddings,
                                                                                    :embedding_vector_binary)
      remove_column :ragdoll_embeddings, :vector_dimensions if column_exists?(:ragdoll_embeddings,
                                                                              :vector_dimensions)
    end
